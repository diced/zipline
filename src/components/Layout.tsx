    ...
    >
      <AppShell.Header px='md'>
        <div style={{ display: 'flex', alignItems: 'center', height: '100%' }}>
          ...
          </Title>          {/* Desktop sidebar toggle button */}
        ...
        </div>      </AppShell.Header>      <AppShell.Navbar hidden={!opened} zIndex={90} className={styles.navbar} style={{ 
        overflow: 'hidden', 
        display: 'flex', 
        ...
        <Box hiddenFrom='sm' p="md" pb="xs">
          ...
          <Divider />
        </Box>        {/* Main navigation section */}
        <ScrollArea flex={1} type="never" className={styles.navbarContent} style={{ 
          padding: navbarCollapsed ? '12px 4px' : '16px 12px'
        }}>
          <Box>
            {navLinks
              .filter((link) => !link.if || link.if(user as Response['/api/user']['user'], config))
              .map((link) => {                if (!link.links) {
                  if (navbarCollapsed) {
                    ...
                    return (                      <div
                      ...
                      >
                        <Box className={styles.iconWrapper} style={{ 
                          fontSize: '1.4rem',
                          display: 'flex',
                          alignItems: 'center',
                    ...
                    return (
                      <Menu key={link.label} position="right-start" offset={12} withinPortal>                        <Menu.Target>
                          <div
                            title={link.label} // Use native HTML title for tooltip
                            style={{ 
                              padding: '12px 8px',
        ...
        <Box className={styles.bottomSection} style={{ 
          ...
          {config.website.externalLinks.length > 0 && (
            <Box>              {config.website.externalLinks.map(({ name, url }, i) => {
                if (navbarCollapsed) {
                  return (                    <div
                      key={i}
