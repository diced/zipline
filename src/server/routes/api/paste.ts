...
export default fastifyPlugin((server: FastifyInstance, _, done) => {
  ...
  server.get<{
    ...
    try {
      ...
      } as PasteInfoResponse);    } catch (error) {
      logger.error('Error getting paste info').error(error as Error);      return res.status(500).send({ error: 'Failed to get paste info' });
    }
  });
  done();
}, { name: PATH });
